...
func Test_ParseValidLines(t *testing.T) {
	scenarios := map[string]struct {
		...
		"movie with bad formatted year": {
			line: []string{"1", "Toy Story (xpto)", "Adventure|Animation|Children|Comedy|Fantasy"},
			expected: loadondb.Movie{
				ID:     1,
				Title:  "Toy Story",
				Year:   0,
...
func Test_ParseInvalidLines(t *testing.T) {
	scenarios := map[string]struct {
		...
		"line with non valid id": {
			line:              []string{"", "Toy Story (xpto)", "Adventure|Animation|Children|Comedy|Fantasy"},
			expectedErrorCode: errors.NonValidID,
		},
		"line with non numeric id": {
			line:              []string{"xpto", "Toy Story (xpto)", "Adventure|Animation|Children|Comedy|Fantasy"},
			expectedErrorCode: errors.NonValidID,
		},
	}
